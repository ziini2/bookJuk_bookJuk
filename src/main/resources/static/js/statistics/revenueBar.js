document.addEventListener('DOMContentLoaded', function () {
    ...
    $('#period').on('change', function () {
        period = $(this).val();
        requestRevenue(period);
    });

    const requestRevenue = function(period) {
        $('.revenue-bar-chart').empty();
        $.ajax({
            url: '/admin/period/revenue',
            method: 'GET',
            data: {
                period: period
            },
            success: function (data) {
                ...
                // SVG 생성
                const svg = d3.select(".revenue-bar-chart")
                    .append("svg")
                    .attr("width", width)
                    .attr("height", height)
                ...
                // 라디오 버튼 이벤트 리스너
                d3.selectAll("input[name='revenue-option']").on("change", function () {
